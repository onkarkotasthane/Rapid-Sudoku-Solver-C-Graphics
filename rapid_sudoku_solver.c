/*
@FILE         		: rapid_sudoku_solver.cpp
@PROGRAM			: RAPID SUDOKU SOLVER
@AUTHOR/PROGRAMMER	: ONKAR V.KOTASTHANED
@VERSION			: 1.0
@WEBSITE			: http://sudokusolver.esy.es
*/
#include<time.h>
#include<ctype.h>
#include<stdlib.h>
...
void main()
{
	int gd=DETECT, gm, i=0, errorcode; 	//DETECTING GRAPHICS DRIVER
	initgraph(&gd,&gm,"bgi//");  		//graphics initialization
	errorcode = graphresult();
	if (errorcode != grOk)  			// IF GRAPHICS NOT OK
	{
...
int drawsudoku()
{
	...
	line(240,1,639,1);			//VGA: 640 x 480 (639 x 479)...
	line(240,479,639,479);		//EGA: 640 X 350
	line(240,1,240,479);
	line(639,1,639,479);
								//SLEEPING LINE...
...
void rss() //RAPID SUDOKU SOLVER LOG PRINTING
{
	settextstyle(10,0,2);
	setcolor(11);
...
int loading() //LOADING/ PROCESSING SCREEN
{
	int i,x=180,y=305,lp=0;
	cleardevice();
	settextstyle(0,0,1);
	outtextxy(270,270,"PROCESSING...");
	line(175,300,450,300); 			// 	VGA/EGA=640 x 480 pixel's  (320 x 240)
...
int print(int col[][3][3]) //PRINTING SUDOKU IN PROPER FORMAT
{
	int j,i,x,y,b,r,c,copcol[81],ctr;
	char a[25];
		cleardevice();
		//**************************************************
		drawsudoku();   //DRAWING SUDOKU IN FORMAT
		//**************************************************
		settextstyle(1,0,2);
		setcolor(11);
	...
	outtextxy(70,50,"-REPORT-");
      /*	outtextxy(0,80,"SUDOKU NO:");
		setcolor(15);
	outtextxy(185,80,sno);
		setcolor(7);
	outtextxy(0,100,"SUDOKU DATE:");
		setcolor(15);
	outtextxy(160,100,sdate);
		setcolor(7); */
...
///////////////////////////////////////////////////////////////////////////////
int printSudo(int prob[][3][3]) //PRINTING SUDOKU
{
	...
	for(b2=0;b2<=2;b2++)
	{
		for(i=0;i<=2;i++)	//row
		{
			...
			{
				for(j=0;j<=2;j++)prob[b][i][j]!=0?printf(" %d",prob[b][i][j]):printf(" -");
				printf(" ");
			}
			printf("\n");
		}
...
int canPut(int prob[][3][3],int block, int row, int col, int num) //CHECKING IF NUMBER CAN BE PUT ON SELECTED FIELD
{
	...
	for(i=0;i<=2;i++)for(j=0;j<=2;prob[block][i][j]==num?flag=0:NULL,j++);
	(block/3)==0?b=0:(block/3)==1?b=3:(b=6);
	for(b2=b+2;b<=b2;b++)for(j=0;j<=2;prob[b][row][j]==num?flag=0:NULL,j++);
	...
	for(b=0;b<=2;b++)for(j=0;j<=2;prob[colset[b]][j][col]==num?flag=0:NULL,j++);
	return flag;
}

int isSolved(int prob[][3][3]) //CHECKING IF SUDOKU IS SOLVED
{
	int flag=1,b1,sum,b,i,j,sudo[3]={0,3,6};
	for(b1=0;b1<=2;b1++)
		for(i=0;i<=2;i++)
		{
			for(sum=0,b=sudo[b1];b<=sudo[b1]+2;b++)for(j=0;j<=2;sum+=prob[b][i][j],j++);
			sum!=45?flag=0:NULL;   //IF SUM IS 45 
		}
	return flag;
}

int solve(int prob[][3][3],int block) //BACKTRACK ALGO
{
...
void mainmenu()
{
	int i;
	cleardevice();
	rss();				//PRINTING RAPID SUDOKU SOLVER ON TOP
						//OUTSIDE BORDER...
	setcolor(2);		//2
	line(0,0,639,0);
	line(0,0,0,479);
...
void inpval()
{
	...
	int evx=1;//'enter values' animation
...
//SCANNING VALUES IN ARRAY(LINER AND GRAPHICS)
	...
	outtextxy(0,0,"RAPID SUDOKU SOLVER");
		settextstyle(5,0,1);
	for(evx=1;ev[evx]!='\0';evx++)     //ENTER VALUES ANIMATION
	{
		gotoxy(evx,5);
		printf("%c",ev[evx-1]);
		delay(100);
	...
	y=15;
		setcolor(7);	//6
	bspw=0;
	copcol[ctr]=9999;        //INITIAL VALUE (SUPPOSE FIRST VALUE IS ALPHABATE)
	for(j=0;j<9;j++)
	{
		x=258;
		for(i=0;i<9;i++)
